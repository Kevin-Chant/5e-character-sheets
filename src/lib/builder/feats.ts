...
// Lives with the feat catalog, not the level-up wizard: creation applies a
// feat too (Variant Human, Custom Lineage).

// Interface rather than `LevelUpState` so creation's level-1 feat application
...
export function applyFeat(
  ...
  const raisedStat = feat.abilityIncrease
    ...
    : undefined;
  // A half-feat's +1 obeys the same stat cap an ASI does.
